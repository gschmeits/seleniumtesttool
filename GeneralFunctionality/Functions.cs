...
namespace GeneralFunctionality
{
    ...
    public class Functions
    {
        ...
        public static void CheckDir(string project)
        {
            var projectPath = GetCurrentDir(1) + project + @"\CSV";

            var pathG = projectPath.Split('\\');
            var g = string.Empty;

            for (var i = 0; i < pathG.Length - 1; i++) g += pathG[i] + "\\";

            projectPath = g;
            var di = new DirectoryInfo(projectPath);
            var dis = new DirectoryInfo(projectPath + "\\CSV");
            var diserror = new DirectoryInfo(projectPath + "\\CSV");
            if (!dis.Exists)
            {
                var driverPathSub = di.CreateSubdirectory("CSV");
                var applicationPathSub = di.CreateSubdirectory("CSV\\");
                var applicationPathSubErrors = di.CreateSubdirectory("CSV\\");
            }
            else
            {
                var applicationPath =
                    new DirectoryInfo(projectPath + "\\CSV");
                if (applicationPath.Exists)
                {
                }
                else
                ...
                applicationPath = new DirectoryInfo(projectPath + "\\CSV");
            ...
            int borderWidth)
        {
            ...
            if (rc.Displayed)
            {
                var visible = IsElementVisible(context);
                if (visible)
                {
